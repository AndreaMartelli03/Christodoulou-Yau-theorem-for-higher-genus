...
\title{An application of Hersch's balancing trick}
...
In these notes, we are going to use the Hersch's balancing trick to extend the following classical result by D. Christodoulou and S.T. Yau.
\begin{thm}[Christodoulou, Yau]\label{thm: Christodoulou-Yau}
    ...
    \]
\end{thm}
The importance of this result is due to a theorem of Huisken and Yau \cite{HuiskenYau_CenterMass}: every asymptotically flat\footnote{A complete 3-manifold \((M,g)\) is \emph{asymptotically flat} (AF in short) if there are a compact subset \(K \subset M\), a positive number \(R>0\) and a diffeomorphism \(\vp \colon \R^3 \setminus B_R(0) \to M\setminus K\) such that if \(g_{ij} \dif x^i\dif x^j = \vp^*g\) on \(\R^3\setminus B_R(0)\), then
\[
    g_{ij}= \left(1+ \frac{2m}{r} \right) \delta_{ij} + O(r^{-2}).
\]
The charts like \((M\setminus K, \vp^{-1})\) are called \emph{asymptotically flat (AF) charts}.} 3-manifold \((M,g)\) admits a foliation of the asymptotically flat end by CMC-stable 2-spheres \(\{\Sigma_r\}_{r>R}\). Then we could define a notion of total mass taking the limit of the Hawking mass of these CMC-stable 2-spheres when \(R \to +\infty\). Turns out that this coincides with the Arnowitt–Deser–Misner (ADM) mass\footnote{
...
\]
Given this facts, as a corollary of Christodoulou-Yau theorem we get the Riemannian positive mass theorem. 

\begin{cor}[Riemannian positive mass theorem]
...
\begin{defi}
    ...
    \] 
    between oriented closed surfaces such that 
    \begin{itemize}
        \item there is a finite set \(\Gamma\subset \Sigma_1\) (whose points are called \emph{branch points}) with finite image \(\Phi(\Gamma)\) and
...
\begin{proof}[Proof of Theorem~\ref{thm: Christodoulou-Yau for higher genus}]
    Let \(\Phi \colon (\Sigma,g) \to (\Sp^2,g_{\Sp^2})\) be a branched conformal map as in Theorem~\ref{thm: corollary of Brill-Noether}. By Lemma~\ref{lem: Hersch trick}, up to consider \(\gamma \circ \Phi\) for some conformal diffeomorphism \(\gamma \in \mathrm{Conf}(\Sp^2,g_{\Sp^2})\), we can assume that 
    \begin{equation*}
    ...
    \begin{proof}[Proof of Claim~\ref{claim: energy and conformal diffeo}]
        ...
        Let's prove \eqref{eq: Riemannian area element under conformal changes}. Fix some coordinates \((U,x^i)\) on \(\Sigma \setminus \Gamma\) and write 
        \[
            g= g_{ij} \dif x^i \otimes \dif x^j, \qquad \Phi^*g_{\Sp^2} = \widetilde{g}_{ij}\dif x^i \otimes \dif x^j.
        \]
        Then,
        \[
...
\]
For each \(t \in (0,\pi/2)\), \(F_t = F(t,\cdot,\cdot)\) is a smooth embedding of \(\Sp^1 \times \Sp^1\) in \(\Sp^3\) and a direct computation shows that
\[
    (F_t)^*g_{\Sp^3} = \cos^2 t \ \dif \theta \otimes \dif \theta + \sin^2 t \ \dif \phi \otimes \dif \phi.
\]
In particular, \(\Sigma_t = F_t(\Sp^1\times \Sp^1)\) is a CMC flat torus in \((\Sp^3,g_{\Sp^3})\) with mean curvature
\[
    H_t = \cot t - \tan t
\]
...
\[
    |\Sigma_t| = (2\pi\cos t)(2\pi \sin t).
\]

Thus:
\begin{align*}
    \frac{64\pi}{3} - H^2_t|\Sigma_t| &= \frac{64\pi}{3} - 4\pi^2 \cos t \sin t (\cot t - \tan t)^2\\
    &= \frac{64\pi}{3} - 4\pi^2 \frac{(\cos^2 t - \sin^2 t)^2}{\cos t \sin t} \to -\infty
\end{align*}
as \(t \to 0\) and as \(t \to \pi/2\).

On the other hand, 
\[
    \frac{2}{3}\int_{\Sigma_t} R^{\Sp^3} + |h_t^\circ|^2 \ \dif \sigma = \frac{16}{3},
\]
so for some \(t \in (0,\pi/2)\) close to \(0\) or to \(\pi/2\),
\[
    \frac{64\pi}{3} - H^2_t|\Sigma_t| < \frac{2}{3}\int_{\Sigma_t} R^{\Sp^3} + |h_t^\circ|^2 \ \dif \sigma . 
\]

Actually, it's not hard to prove that, for every \(t \in (0,\pi/2)\), \(\Sigma_t \subset (\Sp^3,g_{\Sp^3})\) is not CMC-stable. From the second variation formula, for any \(u \in C^\infty(\Sigma_t)\) and associated variation variation \(\Sigma_{t,s} = \{\exp_p(tu(p)\nu_t(p) \ \colon \ p \in \Sigma_t\}\),
\begin{align*}
...
In general, the existence of CMC-stable surfaces with positive genus is obstructed by the assumption of positive curvature.
\begin{prop}
    Let \((M^3,g)\) be a Riemannian 3-manifold and let \(\Sigma^2 \subset M^3\) be volume-preserving stable CMC surface of genus \(k\). Then
    \begin{itemize}
...
\begin{proof}
    ...
    \begin{align*}
        R^M_{1212} &= R^\Sigma_{1212} - h_{11} h_{22} + h_{12} h_{12} = \frac{R^\Sigma}{2} - h_{11}h_{22}\\
        &= \frac{R^\Sigma}{2} - \frac{1}{2}(h_{11} + h_{22})^2 + \frac{1}{2}(h_{11}^2 + h_{22}^2)\\
        &= \frac{R^\Sigma}{2} - \frac{H^2}{2} + \frac{|h|^2}{2}
    \end{align*}
    Writing \(E_3=\nu\) (thus forming an orthonormal basis at \(T_pM\)), since \(\Ric^M \ge 0\),
	\begin{align*}
		\Ric^M(\nu,\nu) + |h|^2 &= R^M_{1313} + R^M_{2323} + 2R^M_{1212} - R^\Sigma +H^2 \\
		&= \Ric^M(E_1,E_1) + \Ric^M(E_2,E_2) - R^\Sigma + H^2 \\
        & \ge -2R^\Sigma
	\end{align*}
	Thus, by \eqref{eq: fund ineq}
	\begin{equation}\label{eq: result of Ros rearrangement trick}
		8\pi d \ge - \int_\Sigma R^\Sigma \ \dif \sigma = -4\pi \chi(\Sigma) = 8\pi(k-1)
	\end{equation}
	by Gauss-Bonnet Theorem.
...
\begin{lemma}\label{lem: Hersch trick +}
...
This extended version can be useful when dealing with minimal surfaces of index 1. Indeed recall that if \(\Sigma^2 \subset (M^3,g)\) is a closed two-sided minimal surface with unit normal \(\nu\), then its second variation is
\begin{align*}
...
\]
It is well known that the first eigenvalue \(\lambda_1\) is simple (i.e. the associated eigenspace is 1-dimensional) and that the corresponding eigenfunctions never vanish, so we may assume that \(\vp_1>0\). Recall that the second eigenvalue is characterized as
\[
...
\begin{equation}\label{eq: eigenvalue ineq}
    \int_\Sigma |\nabla_\Sigma u|^2 \ \dif \sigma \ge \int_\Sigma (\Ric^M(\nu,\nu) + |h|^2)u^2 \ \dif \sigma
\end{equation}
for any \(u \in C^\infty(M)\) such that \(\int_\Sigma u \vp_1 \ \dif \sigma = 0\). 
In general, as for the CMC-stability inequality, we cannot test \eqref{eq: eigenvalue ineq} with constant functions. Then one can combine Theorem~\ref{thm: corollary of Brill-Noether} with Lemma~\ref{lem: Hersch trick +} to find test functions that ‘‘kill'' the gradient part of \eqref{eq: eigenvalue ineq}.
...
We need to show that there exists \(a_0 \in \B^3\) such that \(\Psi(a_0)=0\), as \(\gamma_{a_0}\) would the required conformal diffeomorphism. By triangular inequality, for any \(a \in \B^3\)
\begin{equation*}
	\left| \frac{1}{|\Sigma|}\int_\Sigma \gamma_a \circ \Phi\ \dif \sigma \right| \le \frac{1}{|\Sigma|}\int_\Sigma |\gamma_a \circ \Phi| \dif \sigma  = 1,
\end{equation*}
so \(\Psi(\B^3) \subset \overline{\B^3}\).
...
To prove that \(0 \in \Psi(\B^3)\), we can argue as in the classical proof of Brouwer fixed point theorem. We know that there is no retraction of \(\overline{\B^3}\) on its boundary \(\de \B^3\) (for example, because the homology group \(H_2(\de \B^3) \cong \Z\) cannot be isomorphic to any subgroup of \(H_2(\overline{\B^3})=0\)). Suppose by contradiction that \(0 \notin \Psi(\overline{\B^3})\). Then we can define a retraction \(r \colon \overline{\B^3} \to \de \B^3 \) of \(\overline{\B^3}\) on its boundary as:
	...
	\]
a contradiction. So \(0 \in \Psi(\B^3)\), and this proves the lemma. \qedhere
\end{proof}
...
With the notation above, fix \(r=1\). Using Brill-Noether Theorem, we can find the required map \(\Phi\) of degree \(d\) if and only if
\[
